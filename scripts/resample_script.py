...
def main(
    ...
    if resampled_data_path is not None:
        ...
        export_path = resampled_data_path / f"{participant_id}.hdf5"
        pt_export.export_hdf(export_path, complete_data_structure)
    if fig_path is not None:
        figname = f"resample-comparison-{participant_id}-dt-{T_resample*1000:0.0f}ms"
        save_figure(resample_fig, fig_path, figname, ("png", "svg"))
    if show_plot:
        plt.show()
