...
def export_hdf_from_raw(folder_path: Path, output_path: Path, sub_folders: Iterator[Path], metadata: dict[str, dict | list[dict]], topics: tuple[str, ...] = ("pupil",)):
    ...
    folder_path: pathlib.Path
        ...
        corresponding``.pldata`` and ``.npy`` files.
    output_path: pathlib.Path
        The folder to store the exported HDF5 file. The file will be named based on the
        name of the last folder in ``folder_path``. For example, if 
        ``folder_path = C:/Users/user/recordings/P01/``, then the output file will be
        ``[output_path]/P01.hdf5``.
    sub_folders: Iterator[pathlib.Path]
        An interator or iterable of pathlib.Path objects that specify which subfolders
        within ``folder_path`` should be exported.
    metadata: dict[str, dict | list[dict]]
        A dictionary of metadata with a header dictionary containing metadata about the
        participant and experiment, and a list of dictionaries where element ``i`` 
        contains the data for trial number ``i``.

        See the output of :py:func:`pupiltools.export.get_metadata` for an example.
    topics: tuple[str, ...], default=("pupil",)
        Topics to include in the exported data. Only ``("pupil",)``, ``("gaze",)``, or
        ``("pupil", "gaze")`` are supported.
    """
    export_file = output_path / f"{folder_path.name}.hdf5"
    with h5py.File(export_file, 'w') as f_root:
        assert isinstance(metadata["header"], dict)
